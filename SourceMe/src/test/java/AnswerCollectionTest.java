...
class AnswerCollectionTest {
	...
	void testGetConnection() {
		Connection connection = answer.getConnection(jdbcURL, jdbcUsername, jdbcPassword, jdbcDriver);
		assertNotNull(connection);
		Connection connectionError = answer.getConnection(jdbcURL, usernameErr, passwordErr, jdbcDriver);
		assertNull(connectionError);
		Connection connectionError2 = answer.getConnection(jdbcURL, usernameErr, passwordErr, driverErr);
	...
	void testGetAnswerByQnsId(){
		...
		Connection connectionError3 = answer.getConnection(jdbcURL, usernameErr, passwordErr, driverErr);
...
//		fail("Not yet implemented");
		Question question = answer.showCreateAnswerForm(questionId);
...
//		fail("Not yet implemented");
		Answer currentAnswer = answer.showEditForm(updateId);
...
//		fail("Not yet implemented");
			...
			boolean createError = answer.createAnswer(0, null, postBy, answers);
...
//		fail("Not yet implemented");
		Question currentQuestion = answer.getQuestionById(questionId);
	...
	@Test
	void testEditAnswer() {
		Answer answerByQnsId = (Answer) answer.getAnswerByQnsId(questionId);
		...
		boolean editError = answer.editAnswer(null, postBy, editAnswers, updateId);
	...
	void testDeleteAnswer() {
		List<Answer> answerList = answer.getAnswerByQnsId(questionId);
		assertFalse(answerList.isEmpty());
		int deleteById = answer.deleteAnswer(deleteId);
		assertEquals(answer.getAnswerByQnsId(questionId).size(), answerList.size()-1);
	}
